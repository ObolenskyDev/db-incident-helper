...
# Анализатор логов PostgreSQL для поиска медленных запросов.
# Фильтрует запросы, время выполнения которых превышает заданный порог (ms).
...
def parse_log(filepath, threshold_ms=1000):
    ...
    # Регулярка для формата логов Postgres (duration: N ms)
    ...
    # Сортировка: самые долгие сверху
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        with open(demo_file, "w") as f:
            ...
            f.write("2026-02-15 10:00:05 LOG:  duration: 5400.12 ms  statement: SELECT * FROM klnag_events WHERE id LIKE '%error%';\n")
