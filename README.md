# 🐘 Database Incident Helper

![Bash](https://img.shields.io/badge/Shell-Bash-4EAA25?style=flat&logo=gnu-bash)
![Python](https://img.shields.io/badge/Python-3.8%2B-3776AB?style=flat&logo=python)
![PostgreSQL](https://img.shields.io/badge/DB-PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)
![License](https://img.shields.io/badge/License-MIT-blue.svg)

Набор диагностических утилит для инженеров L3 Support. Предназначен для экстренной диагностики СУБД (PostgreSQL/MariaDB) в инфраструктуре Kaspersky Security Center (Linux). Помогает быстро локализовать причины недоступности консоли KSC или деградации производительности.

---

## 📸 Результат диагностики
*(Демонстрация работы на живом сервере: проверка сервисов и поиск медленных запросов)*

![Execution Result](demo_run.png)

---

## 🛠️ Состав репозитория

### 1. `pg_health.sh` (Bash)
Комплексная проверка «здоровья» инстанса БД.
* **Service Check:** Проверяет статус systemd-юнита (postgresql или mariadb) и выводит аптайм.
* **Port Listener:** Использует утилиту ss для проверки доступности портов (5432/3306). Позволяет с ходу исключить блокировки на уровне фаервола.
* **Disk Usage:** Контролирует свободное место в /var/lib/postgresql. Это критически важный параметр для KSC, так как при переполнении диска база падает в аварийную остановку.

### 2. `slow_query_parser.py` (Python)
Анализатор журналов PostgreSQL (postgresql-*.log) для выявления узких мест в производительности.
* **Log Parsing:** Ищет записи с меткой duration, время выполнения которых превышает заданный порог (по умолчанию 1000 мс).
* **Reporting:** Собирает сводку топ-5 самых тяжелых запросов, из-за которых тормозит работа приложений.
* **Demo Mode:** При запуске без аргументов скрипт сам генерирует тестовый лог, чтобы можно было проверить его работу в безопасной среде.

---

## 🚀 Примеры использования

```bash
# 1. Экспресс-диагностика состояния СУБД
sudo ./pg_health.sh

# 2. Поиск запросов медленнее 2 секунд (2000 мс) в реальном логе
python3 slow_query_parser.py /var/log/postgresql/postgresql-14-main.log 2000

# 3. Демонстрационный запуск (генерация тестовых данных)
python3 slow_query_parser.py
